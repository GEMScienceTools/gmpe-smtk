...
class gmpe_ranking_metrics_wrt_imt_test(unittest.TestCase):
    ...
    parser = ESMFlatfileParser.autobuild("000",
                                         "ranking metrics wrt period test",
                                         output_database, input_fileset)
    ...
    filename = os.path.join(output_directory, 'test.csv')

    # Get loglikelihood and sample of loglikelihood based model weights first
    # wrt period using modified function and then compute average over all
    # considered spectral periods to show match to outputs of original function

    # Get original loglikelihood and sample of loglikelihood based model
    # weights
