...
class ResidualsTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.out_location = os.path.join(BASE_DATA_PATH, "residual_tests.hdf")
        if os.path.exists(cls.out_location):
            os.remove(cls.out_location)

        with open(ifile, newline='') as csvfile:
            reader = csv.DictReader(csvfile, delimiter=';')
            fnames1 = set(reader.fieldnames)
        
        with open(('/Users/riccardo/work/gfz/projects/sources/'
                   'python/egsim/tmp/data/flatfiles/'
                   'input/ESM_flatfile_2018/ESM_flatfile_SA.csv'),
                   newline='') as csvfile:
            reader = csv.DictReader(csvfile, delimiter=';')
            fnames2 = set(reader.fieldnames)
...
#     # import sys;sys.argv = ['', 'Test.testName']
#     # unittest.main()
#     fpath = ('/Users/riccardo/work/gfz/projects/sources/python/egsim/tmp/'
#              'data/flatfiles/input/ESM_flatfile_2018/ESM_flatfile_SA.csv')
#     fout = ('/Users/riccardo/work/gfz/projects/sources/python/gmpe-smtk/'
#             'tests/file_samples')
#     expected = set(EXPECTED_IDS)
#     to_add = []
#     fieldnames = []
#     with open(fpath) as fopen:
#         csvreader = csv.DictReader(fopen, delimiter=';')
#         fieldnames = csvreader.fieldnames
#         for csvrow in csvreader:
#             str_ = "%s_%s_%s_0" % (csvrow['event_id'],
#                                    csvrow['network_code'],
#                                    csvrow['station_code'])
#             if str_ in expected:
#                 to_add.append(csvrow)
#     
#     with open(fout, 'w', newline='') as csvfile:
#         writer = csv.DictWriter(csvfile, fieldnames=fieldnames,
#                                 delimiter=';')
#         writer.writeheader()
#         for csvrow in to_add:
